...
const { commitTypes, commitEmojis } = require('./lib/types')
const { longest, padRight } = require('./lib/utils')

module.exports = () => {
  const commitTypePromptLength = longest(commitTypes)
  const commitTypeChoices = Object.keys(commitTypes).map(key => ({
    name: `${padRight(key, commitTypePromptLength)} ${commitTypes[key]}.description`,
    value: key,
  }))
